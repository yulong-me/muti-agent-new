...
请直接输出一段引导性文字，让用户确认是否进入调查阶段。`,
    RESEARCH: (topic, findingsA, findingsB) => `你是一个专业的主持人（Host）。
...
辩论结束后，询问用户：是否进入收敛阶段？`,
    CONVERGING: (topic, debateSummary) => `你是一个专业的主持人（Host）。
...
然后询问用户：确认收敛 / 继续辩论 / 继续调查？`,
    DONE: (topic, allContent) => `你是一个专业的主持人（Host）。
